""" Taken from https://github.com/chiphuyen/tf-stanford-tutorials/tree/master/assignments/chatbot
Chip Huyen Repo to pre-process Cornell data movie dialogs to train a seq 2 seq network, thank you Chip!,
this works with python 3, just a little decode encode updates
...
DATA_PATH = '/Users/memo/_sublime/TFSequenceToSequence/project/data/cornell movie-dialogs corpus'
...
def prepare_dataset(questions, answers):
    ...
    for filename in filenames:
        files.append(open(os.path.join(PROCESSED_PATH, filename),'wb'))

    for i in range(len(questions)):
        if i in test_ids:
            files[2].write(questions[i].encode() + b'\n')
            files[3].write(answers[i].encode() + b'\n')
        else:
            files[0].write(questions[i].encode() + b'\n')
            files[1].write(answers[i].encode() + b'\n')
...
def basic_tokenizer(line, normalize_digits=True):
    ...
    line = re.sub('<u>', '', line.decode("ISO-8859-1"))
    line = re.sub('</u>', '', line)
    line = re.sub('\[', '', line)
    line = re.sub('\]', '', line)
    words = []
...
def load_data(enc_filename, dec_filename, max_training_size=None):
    encode_file = open(os.path.join(PROCESSED_PATH, enc_filename), 'rb')
    decode_file = open(os.path.join(PROCESSED_PATH, dec_filename), 'rb')
